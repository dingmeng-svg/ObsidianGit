...
| `财务` | 投资、理财、记账、资产 | |
| `技术` | 编程、架构、AI、工具 | |
| `心理学` | 心理现象、心理模型 | 细分标签，父领域：个人成长 |
| `知识管理` | 笔记方法、知识体系 | 细分标签，父领域：个人成长 |
| `语言` | 语言习得、外语学习、语言学、翻译 | |
| `健身` | 训练方案、动作标准、康复拉伸 | 细分标签，父领域：生活 |

> 其中 **4 个细分标签**：`心理学`（父：个人成长）、`知识管理`（父：个人成长）、`健身`（父：生活）。它们属于 10 个核心标签之内，不单独计数。
...
| `素材` | 外部摘抄、灵感收集 | 类型标记 |
| `日记` | 仅限 Daily Log | 类型标记 |
| `习惯` | 习惯方法论专用 | 类型标记 |
| `MOC` | MOC 索引文件专用 | 类型标记 |
| `洞察` | 从复盘/阅读中提炼的独立认知产物 | 类型标记 |
| `复盘` | BuJo 体系内周/月/季/年度/Future Log | 类型标记 |
...
|:---|:---|
| **只能从 20 种中选择** | 不可自创标签 |
| **细分标签与父标签互斥** | 打细分标签不打父标签（例：打 `健身` 不打 `生活`） |
| **属性不进入 tags** | 拉伸/松解/瑜伽/呼吸等属性由 L3 特化字段（如 `fit_type`）承载 |
| **功能性标签不计入双标签统计** | 仅用于检索分类 |
| **`日记` 仅限 Daily Log** | Weekly/Monthly/Quarterly/Annual/Future Log 使用 `复盘` |
| **模板文件本身用 `系统文档`** | 生成物独立打标签，不由模板继承 |
...
| 字段 | 说明 | 格式 |
|:---|:---|:---|
| `source` | 核心主来源（仅 1 个） | 来源名称，非URL |
| `author` | 原作者 | 作者名 |
| `project` | 归属项目名 | 纯文本，不加双链 |
| `people` | 关联人物 | 人名，如 `丁萌`、`杨媚`、`袁野` |
| `aliases` | 搜索别名 | 必须使用引号包裹每个别名：`["别名1", "别名2"]` |
| `related` | 关联笔记 | 必须使用 YAML 列表格式，每个双链用引号包裹：`- "[[笔记名]]"` |
...
| 字段 | 适用笔记类型 | 说明 |
|:---|:---|:---|:---|
| `human_edited` | 全库 | **权限锁**。`true` = 禁止 Agent 覆盖正文。与内容质量无关。若笔记 YAML 中无此字段，视为 `false` |
| `deep_reviewed` | 仅 `31_Atomic_Notes/语言/` 笔记 | **质量标记**。`true` = 内容经人工深度校验（至少一轮深回流修正）。`human_edited` 管权限，`deep_reviewed` 管质量，语义正交 |
| `ai_generated` | AI 编译产物 | 标记 AI 生成 |
| `last_ai_edit` | Agent 修改过的笔记 | 最近 AI 修改日期 |
| `ai_edit_summary` | Agent 修改过的笔记 | AI 修改内容简述 |
| `ingest_status` | `00_Inbox/_raw/` 原素材 | 编译状态。枚举值：`pending`（待处理）/ `ingested`（已编译）/ `skipped`（跳过）/ `archived`（已归档）。不计入 META-02 配额 |
...
| `version` | 系统文档 | 语义化版本号。`v0.0.0` 为草稿，正式启用须升级至 `v1.0.0`。**非系统文档禁止使用** |
| `last_sync` | 系统文档 | 最后与上游真值源规则对齐的日期，非文件修改时间 |
| `related_docs` | 系统文档 | 关联文档的双链列表 |
| `dependent_skills` | 系统文档 | 依赖本文档的 Skill 列表 |
| `insight_type` | 04_Insight 模板 | 周期洞察类型。取值：`insight_report` |
| `period_type` | 04_Insight 模板 | 周期类型。取值：`daily`/`weekly`/`monthly`/`quarterly`/`annual` |
| `period_range` | 04_Insight 模板 | 周期覆盖范围，仅用于人眼识别 |
| `daily_promise` | 04_Insight · Daily 模板 | 每日唯一承诺，用于周度候选池聚合 |
| `long_term_project` | 项目主文档 | 布尔值。豁免 draft 超 30 天 P0 报错，仅限项目主文档 |
| `lint_ignore` | 全库 | 字符串数组。标记已知问题且不参与 Lint 报告 |
...
|| 健身方案 | `fit_type, fit_content_type, fit_cycle, target_muscle, fit_level, train_duration, train_frequency, fit_goal, rest_rule, equipment, injury_note, diet_strategy, demo_gif, training_date` | 仅 `20_Areas/23_生活与健康/健身/` 及 `30_Resources/31_Atomic_Notes/健身/` |

### 溯源字段标准化

| 字段 | 适用场景 | 格式 | 是否必填 |
|:---|:---|:---|:---|
| `source` | 原始出处 | 单一来源名称（强制单值）。翻译类素材填写原著作名，译者/译本信息下沉正文 `## 参考来源` 区块。多源素材的次要来源放正文，不占用 source | 推荐 |
...
| `tags: [拉伸]` 或 `tags: [松解, 瑜伽]` | `tags: [健身]`，属性下沉到 `fit_type` 等特化字段 | 属性错置（错把内容属性当领域标签） |
...
| 用户想用"拉伸/松解/瑜伽"等词作为标签 | `tags: [拉伸]` 等 | 解释 `tags` 只承载领域归属，具体属性由 L3 特化字段（如 `fit_type`）承担 | 查阅 `13_健身方案编写标准` 的"非力量训练分类映射矩阵" |
