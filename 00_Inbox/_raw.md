---
title: _raw
date: 2026-07-09
tags:
  - MOC
  - Inbox
  - Raw
doc_level: L4
aliases:
  - Raw 素材 MOC
---

```dataviewjs
const total = dv.pages('"00_Inbox/_raw"').where(p => p.file.name != "_raw 素材总MOC").length;
const pending = dv.pages('"00_Inbox/_raw"').where(p => p.file.name != "_raw 素材总MOC" && p.ingest_status != "ingested").length;
const done = total - pending;
dv.span(`📊 总素材 ${total}  ·  🔴 待处理 ${pending}  ·  🟢 已处理 ${done}`);
```

# 🗄️ 原料仓总览

> 系统自动 Ingest 的原始素材。只读不改，永久存档。
>
> 处理流程：外部导入 → 进入 `_raw/` → Hermes Agent 自动 Ingest → 提炼为原子笔记或归档。

## 🔴 待处理（等待 Ingest）

```dataview
TABLE file.ctime AS "创建时间", file.mtime AS "修改时间"
FROM "00_Inbox/_raw"
WHERE file.name != "_raw 素材总MOC" AND (ingest_status != "ingested" OR ingest_status = null)
SORT file.ctime ASC
```

## 🟢 已处理（Ingest 完成）

```dataview
TABLE file.ctime AS "创建时间", ingested_at AS "Ingest 完成"
FROM "00_Inbox/_raw"
WHERE ingest_status = "ingested"
SORT file.mtime DESC
LIMIT 10
```

## 📦 全部原始素材

```dataview
TABLE file.ctime AS "创建时间", file.mtime AS "修改时间"
FROM "00_Inbox/_raw"
WHERE file.name != "_raw 素材总MOC"
SORT file.ctime DESC
```

---

*💡 使用 `ingest_status: ingested` 标记已 Ingest 的素材。文件只读不改，Ingest 产物写入 `31_Atomic_Notes/`。*